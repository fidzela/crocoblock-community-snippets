...
class CustomerSubscriptionUpdated {
	...
	public function handle( array $payload ): WP_REST_Response {
		...
		try {
			...
			$mercadopago_sub_id = (string) ( $sub['id'] ?? '' );

			if ( '' === $mercadopago_sub_id ) {
				error_log( '[MercadopagoWebhook][customer.subscription.updated] Missing subscription id in payload' );
				return new WP_REST_Response( array(
					'status' => 'ok',
					'action' => 'noop',
					'reason' => 'no_mercadopago_subscription_id',
				), 200 );
			}
			...
			$q     = SubscriptionsView::find( array( 'billing_id' => $mercadopago_sub_id ) )->query();
			...
			try {
				...
				error_log( '[MercadopagoWebhook][customer.subscription.updated] Local update failed: ' . $e->getMessage() );
				...
				return new WP_REST_Response( array(
					...
					'mercadopago_subscription'  => $mercadopago_sub_id,
				), 200 );
			}
			...
			error_log( '[MercadopagoWebhook][customer.subscription.updated] fatal: ' . $e->getMessage() );
