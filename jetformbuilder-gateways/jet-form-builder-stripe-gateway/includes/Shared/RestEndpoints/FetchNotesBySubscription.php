...
class FetchNotesBySubscription extends Rest_Api_Endpoint_Base {

	use Traits\Paginated_Args;
	use Dynamic_Rest_Url_Trait;

	public static function get_rest_base() {
		return 'subscription/notes-box/(?P<id>[\d]+)';
	}
	...
	public function run_callback( \WP_REST_Request $request ) {
		$box  = ( new SubscriptionNotesBox() )->set_id( $request->get_param( 'id' ) );
		...
		try {
			...
			return new \WP_REST_Response(
				array(
					'message' => __( 'Notes not found', 'jet-form-builder' ),
				),
				404
			);
		}
