...
- ✅ **FEITO (v2.0.19–v2.0.20):** `record()` transacional (§8.2 — Payment + SubscriptionToPayment atômicos; enriquecimento do pagador segue best-effort fora); alerta de `amount mismatch` **sempre-ligado** (§11.2, `WebhookConfig::audit()`); trilho de auditoria mínimo **sempre-ligado** (§14.1 — uma linha por webhook no Dispatcher: topic/data_id/status/result, filtrável); docblock fail-open alinhado (§9.5); **refund ENCERRA a assinatura (§12.4/§12.5)** — decisão do dono: estorno de cobrança **cancela a preapproval no MP + marca a assinatura CANCELLED**, idempotente, nos DOIS caminhos (admin + webhook) via `Subscription_Refund_Closer`. O cancel no MP é **gate**: se falhar, NÃO marca local terminal (evita §12.5). Pay-now → no-op. `PreapprovalNotification` ganhou guard de terminal no topo (o `cancelled` auto-induzido não re-dispara evento).
- ⏳ **Pendente — decisão do dono:** multi-conta (§9.8 — assumir 1 conta MP por site?); fail-open vs enforce (§9.4).
- ✅ **FEITO (v2.0.21–v2.0.24):** **reconciliador WP-Cron** (§3.3/§7.4 — varre APPROVAL_PENDING/CREATED do MP e reconcilia REUSANDO os handlers do webhook; teto de leitura de 200 + idade máx. de 7 dias + grace de 15min; flag `JFB_MP_RECONCILER_ENABLED`); **re-sync dos termos** quando o plano muda no MP (PreapprovalNotification atualiza o RecurringCyclesModel local); **flag de efeitos pendentes** (payments_meta — identificável via auditoria/consulta + reexecutável MANUAL pelo hook `jet-form-builder/mercadopago/rerun-effects`, pois ações do form não são idempotentes).
- ⏳ **Pendente — adiado com justificativa:** índice em `billing_id` (§10.2 — schema da lib Shared + migração; ganho baixo numa tabela pequena); janela de `ts`/replay (§9.2/§9.3 — comportamento do `ts` por retry do MP não confirmado); multi-conta (§9.8 — decisão do dono: **1 conta MP por site** por ora). Ver `QA-CENARIOS.md` para o próximo ciclo de questionamentos.
