	...
	class Jet_Engine_CPT_Meta {
		...
		public function prepare_meta_fields( $meta_box = array() ) {
			...
			foreach ( $meta_box as $field ) {
				...
				$result[ $field['name'] ] = array(
					...
					'field_layout'    => ! empty( $field['field_layout'] ) ? $field['field_layout'] : 'inline',
					'name'            => $field['name'],
					'hide_field_name' => ! empty( $this->hide_field_names ),
					'element'         => 'control',
					'title'           => apply_filters( 'jet-engine/compatibility/translate-string', $title ),
				...
				switch ( $field['type'] ) {
					case 'taxonomy':
						...
						$result[ $field['name'] ] = apply_filters( 'jet-engine/meta-fields/taxonomy/args', $result[ $field['name'] ], $field, $this );

						// Defensive fallback: if taxonomy runtime handler was not attached for any reason,
						// force a supported control type so field still renders in UI.
						if ( empty( $result[ $field['name'] ]['type'] ) || 'taxonomy' === $result[ $field['name'] ]['type'] ) {
							$result[ $field['name'] ]['type'] = 'select';
						}

						if ( ! isset( $result[ $field['name'] ]['options'] ) || ! is_array( $result[ $field['name'] ]['options'] ) ) {
							$result[ $field['name'] ]['options'] = array();
						}
		...
		public function prepare_field_conditions( $field = array(), $all_fields = array() ) {
			...
			foreach ( $field['conditions'] as $condition ) {
				...
				$is_multi_select = ( in_array( $condition_field['type'], array( 'select', 'taxonomy' ), true ) )
									&& (
										( ! empty( $condition_field['is_multiple'] ) && filter_var( $condition_field['is_multiple'], FILTER_VALIDATE_BOOLEAN ) )
										|| ( ! empty( $condition_field['multiple'] ) && filter_var( $condition_field['multiple'], FILTER_VALIDATE_BOOLEAN ) )
									);
