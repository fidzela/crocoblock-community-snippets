...
if ( ! class_exists( 'Jet_Engine_Dynamic_Tags_Manager' ) ) {
	...
	class Jet_Engine_Dynamic_Tags_Manager {
		...
		protected $class_prefix = 'elementor-dcss-';
		...
		function __construct() {
			...
			add_filter( 'jet-engine/listing/item-classes', array( $this, 'modify_listing_item_class' ), 10, 4 );
		}
		...
		public function modify_listing_item_class( $class, $object, $i, $render ) {
			if ( $render->view !== 'elementor' ) {
				return $class;
			}

			$class[] = $this->class_prefix . jet_engine()->listings->data->get_current_object_unique_id( $object );
		...
		public function add_listing_item_dynamic_css( $content, $listing_id, $inner_templates ) {
			...
			$unique_post_id = jet_engine()->listings->data->get_current_object_unique_id( $post );
			$unique_id      = empty( $unique_id ) ? $unique_post_id : $unique_post_id . '-' . $unique_id;

			foreach ( $post_ids_for_data as $post_id_for_data ) {

				$css_file = $this->get_dynamic_css_file( $post_id, $post_id_for_data, $unique_id );
				$css_file->set_listing_unique_selector(
					sprintf(
						':is( .jet-listing-dynamic-post-%1$s:not( [class*="%3$s"] ), .%3$s%2$s )',
						$post_id,
						jet_engine()->listings->data->get_current_object_unique_id( $post ),
						$this->class_prefix
					)
				);
