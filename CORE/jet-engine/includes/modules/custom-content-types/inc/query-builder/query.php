...
class CCT_Query extends \Jet_Engine\Query_Builder\Queries\Base_Query {
	...
	public function get_preserve_ids_order_by( $args = [] ) {
		...
		if ( is_array( $ids ) ) {
			...
			$ids = array_map( 'absint', explode( ',', (string) $ids ) );
		}
	...
	public function update_args_row( $row ) {
		...
		foreach ( $this->final_query['args'] as $index => $existing_row ) {
			if ( ( isset( $existing_row['field'] ) && isset( $row['field'] ) ) && $existing_row['field'] === $row['field'] && $existing_row['operator'] === $row['operator'] ) {
				...
				// Temporary fix: normalize values and apply intersection for `_ID IN` case.
				// See issue: https://github.com/Crocoblock/issues-tracker/issues/18669
				// May be revised/removed in future if it causes conflicts in other scenarios.
				if (
					$row['operator'] === 'IN' &&
					$row['field'] === '_ID' &&
					! empty( $existing_row['value'] )
				) {

					$existing = is_array( $existing_row['value'] )
						? $existing_row['value']
						: explode( ',', $existing_row['value'] );

					$incoming = (array) $row['value'];

					$row['value'] = array_values(
						array_intersect( $existing, $incoming )
					);
				}
