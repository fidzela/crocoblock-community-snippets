<div>
	...
	></cx-vui-input>
	<cx-vui-select
		label="<?php _e( 'Field Layout', 'jet-engine' ); // phpcs:ignore ?>"
		description="<?php _e( 'Choose how the label is displayed relative to the field', 'jet-engine' ); // phpcs:ignore ?>"
		:wrapper-css="[ 'equalwidth' ]"
		size="fullwidth"
		:options-list="[
			{
				value: 'inline',
				label: '<?php _e( 'Inline (label and field in one row)', 'jet-engine' ); // phpcs:ignore ?>'
			},
			{
				value: 'stacked',
				label: '<?php _e( 'Stacked (label above the field)', 'jet-engine' ); // phpcs:ignore ?>'
			}
		]"
		:value="field.field_layout || 'inline'"
		@input="setFieldProp( 'field_layout', $event )"
		:conditions="getFilteredFieldConditions( [
		{
			'input':   field.object_type,
			'compare': 'equal',
