...
class Query {
	...
	public function maybe_setup_load_more_prop( $query ) {
		...
		if ( ! empty( $_REQUEST['query']['filtered_query'] ) ) {
			$filtered_query = $this->sanitize_filtered_query_args( $_REQUEST['query']['filtered_query'] );
	...
	protected function sanitize_filtered_query_args( $filtered_query ) {

		if ( ! is_array( $filtered_query ) ) {
			return [];
		}

		$disallowed_arguments = array(
			'custom_table_query',
			'fields',
			'suppress_filters',
			'cache_results',
		...
		foreach ( $disallowed_arguments as $argument ) {
			if ( isset( $filtered_query[ $argument ] ) ) {
				unset( $filtered_query[ $argument ] );
			}
		}
