...
class Factory {
	...
	public function __construct( $args = array(), $type = false ) {
		...
		if ( $this->is_user_store() && isset( $this->args['is_cct'] ) ) {
			$this->args['is_cct'] = false;
		}
	...
	public function get_post_count( $post_id = null ) {
		...
		if ( $this->is_user_store() ) {
			return absint( get_user_meta( $post_id, $this->count_posts_key . $this->get_slug(), true ) );
		}
	...
	public function increase_post_count( $post_id ) {
		...
		$custom_increase = apply_filters(
			'jet-engine/data-stores/custom-count-increased',
			false, $post_id, $count, $this
		);

		if ( ! $custom_increase ) {
			if ( $this->is_user_store() ) {
				update_user_meta( $post_id, $this->count_posts_key . $this->get_slug(), $count );
			} else {
