...
/**
 * Safely unserialize value without instantiating arbitrary PHP objects.
 *
 * @param  mixed   $value   Input value.
 * @param  string  $context Optional context for filtering allowed classes.
 * @return mixed            Unserialized value, or original value if unserialization is not needed.
 */
function jet_engine_safe_unserialize( $value = '', $context = '' ) {
	if ( ! is_string( $value ) || ! is_serialized( $value ) ) {
		return $value;
	}

	$allowed_classes = apply_filters(
		'jet-engine/safe-unserialize/allowed-classes',
		array( 'stdClass' ),
		$context,
		$value
	);
...
function jet_engine_maybe_unserialize( $value = '', $context = '' ) {
	...
	return jet_engine_safe_unserialize( $value, $context );
}
...
function jet_engine_maybe_unserialize_object_props( $object ) {
	...
	foreach ( get_object_vars( $object ) as $var => $value ) {
		$unserialized = jet_engine_safe_unserialize( $value, 'object_props' );
